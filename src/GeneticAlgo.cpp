...
void geneticAlgo(Population& population, const Graphe& graphe, const Choix choix){
    int k=0;
    int count = EVOLUTION;
    const int q = population.getTaille()/2;
    const int p = population.getTaille(); // taille initiale

    while (++k)
    {   
        cout<<"---------------ite: "<<k<<endl;
        // generate reproducteurNext
        Population reproducteurNext(series(p-1)*2); //4 * p
        permutation( p, population, graphe, reproducteurNext);
        cout << "reproducteurNext"<<endl << reproducteurNext<<endl;

        // selection reproducteur
        reproducteurNext = selection(choix, q, reproducteurNext);
        cout<<"après selection, reproducteurNext"<<endl<<reproducteurNext<<endl;
        cout<<"here"<<endl;
        // we keep the p smallest individus

        // selection population finale
